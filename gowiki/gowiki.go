...
import (
	"discord_bot/gowiki/cache"
	"discord_bot/gowiki/page"
	"discord_bot/gowiki/utils"
	"errors"
	"fmt"
	"strconv"
	"time"
...
func GetRandomPages(limit int) ([]page.WikipediaPage, error) {
	if limit < 0 {
		limit = 5
	}
	args := map[string]string{
	...
	res, err := utils.WikiRequester(args)
	if err != nil {
		return []page.WikipediaPage{}, err
	}
	if res.Error.Code != "" {
		return []page.WikipediaPage{}, errors.New(res.Error.Info)
	}

	result := make([]page.WikipediaPage, 0, len(res.Query.Random))
	for _, s := range res.Query.Random {
		pge, _ := page.MakeWikipediaPage(-1, s.Title, "", true)
		result = append(result, pge)
	}
	return result, nil
}
